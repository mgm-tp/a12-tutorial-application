...
async function getContactsByType(): Promise<Aggregation.AggregationTuple[]> {
    ...
    // The cast is needed because Data Services sets "projectionName" and "paging"
    // for this endpoint itself, so we must not send them.
    const queryRoot = {
        // The document model we are interested in
        targetDocumentModel: "Contact_Dc",
    ...
    const request: RestRequestPayload = {
        ...Aggregation.Request.build({ queryRoot }),
        // "CustomerType" is a localized enumeration, so the endpoint needs the locale.
        // The "Accept" and "Content-Type" headers are added by the Server Connector.
